...
public final class Logger {
  private static final String DEFAULT_TAG = "BIM";
  ...
  public static void init(Context context) {
    if (fileOutputStream == null) {
      try {
        Date date = new Date();
        String string = ((1900+date.getYear())+"_"+(1+date.getMonth())+"_"+date.getDate()+"_"+date.getHours()+"_"+date.getMinutes()+"_"+date.getSeconds());
        String filePath = FileUtils.getFilePath(context, "log", "log_"+string+".txt");
  ...
  public void v(final String format, final Object... args) {
    if (isLoggable(Log.VERBOSE)) {
      ...
      if (fileOutputStream != null) {
        try {
          fileOutputStream.write((new Date() + " " + toMessage(format, args)+"\n").getBytes());
        } catch (Exception exception) {
          exception.printStackTrace();
        }
      }
