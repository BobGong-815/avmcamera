...
//    long turnLampChangeTime = 0;
    ...
    private boolean isChangeFx = false;//是否是改变方向
    private final CanManager.onSignalValueChangedListener mOnSignalValueChangedListener = (vehicleId, value) -> {
        ...
        if (vehicleId == AVM_UINM_TURN_LIGHT_SW_ST) { //转向激活
            ...
            if (value instanceof Integer) {
                ...
                if (AvmApp.getInstance().getCameraView().isFullWin || isChangeFx) {
                    isChangeFx = true;
            ...
            if(isChangeFx){
                KLog.d("lightChange 执行切换视角的逻辑不处理灯光逻辑");
                return;
            }
            ...
            if (vehicleId == CLUSTER_LEFT_TURN_LAMP && leftLightSt == 1 && rightLightSt == 0 && isCanShowCard) {
                ...
                if (rlTurnTime == 0) {
                    mHandler.postDelayed(() -> {
                        mHandler.removeCallbacksAndMessages("closeWin");
                        isCanShowCard = false;
                ...
                if (rlTurnTime != 0) {
                    ...
                    mHandler.removeCallbacksAndMessages("closeWin");
                    mHandler.postDelayed(() -> AvmApp.getInstance().getCameraView().getViewModel().turnLampChange(0, 0), "closeWin", 1000);
                }
